...
const mailsURL = SERVER_URL + "/CustomerInfo/GetMessagesByCustomerId?userId=";
...
export class MailService implements Resolve<any> {
    ...
    getMailsByFolder(handle): Promise<Mail[]> {
        return new Promise((resolve, reject) => {
            const url = mailsURL + this.currentUser.Id;
            this._httpClient.get(url).subscribe((mails: any) => {
                mails = mails.data;
